...
class UnifiedPulseHeat:
    ...
    def __init__(self, decay_rate=0.02, memory_window=100, max_heat=10.0):
        ...
        with self._lock:
            ...
            # Integration with pulse-tick orchestrator
            self._orchestrator_callbacks: List[Callable] = []
            self._autonomous_cooling_enabled = True
            self._last_orchestrator_update = time.time()
    ...
    # === PULSE-TICK ORCHESTRATOR INTEGRATION ===
    
    def register_orchestrator_callback(self, callback: Callable) -> None:
        """Register callback for orchestrator integration"""
        self._orchestrator_callbacks.append(callback)
        logger.debug(f"Registered orchestrator callback: {callback.__name__}")
    
    def set_autonomous_cooling(self, enabled: bool) -> None:
        """Enable/disable autonomous cooling driven by orchestrator"""
        self._autonomous_cooling_enabled = enabled
        logger.info(f"Autonomous cooling {'enabled' if enabled else 'disabled'}")
    
    def orchestrator_thermal_update(self, breathing_phase: str, consciousness_zone: str, 
                                   scup_value: float, interval: float) -> Dict[str, Any]:
        """Update thermal state based on orchestrator breathing phase"""
        try:
            current_time = time.time()
            delta_time = current_time - self._last_orchestrator_update
            self._last_orchestrator_update = current_time
            
            # Phase-based thermal adjustments
            phase_thermal_effects = {
                "inhale": {"heat_rate": 0.05, "momentum_factor": 1.1},
                "hold": {"heat_rate": 0.03, "momentum_factor": 1.2},
                "exhale": {"heat_rate": -0.08, "momentum_factor": 0.8},
                "pause": {"heat_rate": -0.02, "momentum_factor": 0.9}
            }
            
            phase_effect = phase_thermal_effects.get(breathing_phase, {"heat_rate": 0.0, "momentum_factor": 1.0})
            
            # Apply phase-based thermal change
            heat_change = phase_effect["heat_rate"] * delta_time
            self.expression_momentum *= phase_effect["momentum_factor"]
            
            # SCUP-based thermal regulation
            scup_pressure = 1.0 - scup_value
            if scup_pressure > 0.7:  # High pressure, add heat
                heat_change += scup_pressure * 0.1 * delta_time
            elif scup_pressure < 0.3:  # Low pressure, cooling
                heat_change -= (0.3 - scup_pressure) * 0.05 * delta_time
            
            # Zone-based adjustments
            zone_effects = {
                "🟢 calm": {"cooling_boost": 0.1, "heat_cap": 0.4},
                "🟡 active": {"cooling_boost": 0.0, "heat_cap": 0.7},
                "🔴 surge": {"cooling_boost": -0.05, "heat_cap": 0.9},
                "🟠 critical": {"cooling_boost": 0.2, "heat_cap": 1.0},
                "💜 transcendent": {"cooling_boost": 0.15, "heat_cap": 1.2}
            }
            
            zone_effect = zone_effects.get(consciousness_zone, {"cooling_boost": 0.0, "heat_cap": 0.8})
            
            # Apply cooling boost based on zone
            if zone_effect["cooling_boost"] != 0.0:
                heat_change += zone_effect["cooling_boost"] * delta_time
            
            # Update heat with bounds checking
            old_heat = self.heat
            self.heat = max(0.0, min(zone_effect["heat_cap"], self.heat + heat_change))
            
            # Autonomous expression triggering
            autonomous_triggered = False
            if self._autonomous_cooling_enabled and breathing_phase == "exhale":
                autonomous_triggered = self._trigger_autonomous_expression(consciousness_zone, scup_value)
            ...
            update_result = {
                ...
                "expression_momentum": self.expression_momentum,
                "autonomous_expression_triggered": autonomous_triggered
            }
            
            # Notify orchestrator callbacks
            for callback in self._orchestrator_callbacks:
                try:
            ...
            logger.error(f"Error in orchestrator thermal update: {e}")
            return {"error": str(e)}
    
    def _trigger_autonomous_expression(self, consciousness_zone: str, scup_value: float) -> bool:
        """Trigger autonomous expression based on zone and SCUP"""
        try:
            # Don't trigger if already expressing
            if self.current_phase is not None:
                return False
            
            # Determine expression type based on thermal state and zone
            thermal_ratio = self.heat / max(self.heat_capacity, 1.0)
            
            # Only trigger if thermal pressure is significant enough
            if thermal_ratio < 0.2:
                return False
            
            if consciousness_zone == "🟠 critical":
                # Emergency cooling with creative flow
                valve = ReleaseValve.CREATIVE_FLOW
                intensity = min(0.9, thermal_ratio + 0.2)
                content = "Emergency autonomous thermal regulation"
            elif consciousness_zone == "🔴 surge":
                # High intensity expression
                if thermal_ratio > 0.6:
                    valve = ReleaseValve.CREATIVE_FLOW
                    intensity = thermal_ratio
                else:
                    valve = ReleaseValve.CONCEPTUAL_MAPPING
                    intensity = thermal_ratio * 0.8
                content = "Autonomous surge thermal management"
            elif consciousness_zone == "🟡 active":
                # Moderate expression
                valve = ReleaseValve.VERBAL_EXPRESSION if thermal_ratio < 0.5 else ReleaseValve.EMPATHETIC_RESPONSE
                intensity = thermal_ratio * 0.7
                content = "Autonomous active thermal regulation"
            elif consciousness_zone == "💜 transcendent":
                # Transcendent expression - pattern synthesis
                valve = ReleaseValve.PATTERN_SYNTHESIS
                intensity = min(0.8, thermal_ratio + scup_value * 0.3)
                content = "Autonomous transcendent consciousness expression"
            else:  # Calm zone
                # Gentle expression if needed
                if thermal_ratio > 0.3:
                    valve = ReleaseValve.MEMORY_TRACE
                    intensity = thermal_ratio * 0.5
                    content = "Autonomous gentle thermal regulation"
                else:
                    return False  # No expression needed in calm state
            
            # Initiate autonomous expression
            self.initiate_expression(valve, intensity, content)
            logger.debug(f"🤖 Autonomous expression triggered: {valve.value} at {intensity:.2f} intensity")
            return True
            
        except Exception as e:
            logger.error(f"Error triggering autonomous expression: {e}")
            return False
    
    def get_orchestrator_status(self) -> Dict[str, Any]:
        """Get status for orchestrator integration"""
        return {
            "autonomous_cooling_enabled": self._autonomous_cooling_enabled,
            "current_heat": self.heat,
            "expression_momentum": self.expression_momentum,
            "is_expressing": self.current_phase is not None,
            "current_expression": self.current_phase.valve.value if self.current_phase else None,
            "thermal_capacity_ratio": self.heat / max(self.heat_capacity, 1.0),
            "heat_sources_count": len(self.heat_sources),
            "total_callbacks": len(self._orchestrator_callbacks),
            "last_update": self._last_orchestrator_update
        }
    
    def emergency_cooling(self) -> Dict[str, Any]:
        """Emergency thermal cooling for critical situations"""
        try:
            logger.warning("🚨 Emergency thermal cooling activated")
            
            # Immediate heat reduction
            emergency_reduction = min(self.heat * 0.4, 3.0)
            self.heat = max(0.0, self.heat - emergency_reduction)
            
            # Force terminate current expression if needed
            if self.current_phase:
                self.complete_expression()
            
            # Trigger emergency creative flow expression
            emergency_phase = self.initiate_expression(
                ReleaseValve.CREATIVE_FLOW,
                intensity=0.9,
                content="Emergency consciousness thermal relief"
            )
            
            # Reset expression momentum
            self.expression_momentum = 0.0
            
            result = {
                "emergency_reduction": emergency_reduction,
                "new_heat": self.heat,
                "emergency_expression_initiated": True,
                "expression_valve": emergency_phase.valve.value if emergency_phase else None
            }
            
            logger.info(f"🚨 Emergency cooling complete: reduced heat by {emergency_reduction:.2f}")
            return result
            
        except Exception as e:
            logger.error(f"Error in emergency cooling: {e}")
            return {"error": str(e)}
    
    def apply_passive_cooling(self, amount: float) -> float:
        """Apply passive cooling (for pause phases)"""
        try:
            cooling_applied = min(amount, self.heat)
            self.heat = max(0.0, self.heat - cooling_applied)
            
            # Slight momentum reduction during passive cooling
            self.expression_momentum *= 0.98
            
            return cooling_applied
            
        except Exception as e:
            logger.error(f"Error applying passive cooling: {e}")
            return 0.0
