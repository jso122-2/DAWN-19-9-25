...
try:
    from .zone_tracker import PulseZoneTracker, PulseZone, PulseEvent
except ImportError:
    ...
    class PulseEvent:
        def __init__(self, event_type="update"):
            self.event_type = event_type

try:
    from .state_timer import PulseStateTimer
except ImportError:
    class PulseStateTimer:
        def __init__(self):
            pass

try:
    from .overheat_detector import PulseOverheatDetector, ThermalEvent, ThermalAlert
except ImportError:
    class PulseOverheatDetector:
        def __init__(self):
            pass
    
    class ThermalEvent:
        def __init__(self, event_type="normal"):
            self.event_type = event_type
...
class PulseEngine:
    ...
    def __init__(self, config_path: str = 'config/pulse_config.yaml'):
        ...
        # Log pulse engine initialization
        if TELEMETRY_AVAILABLE:
            log_event('pulse_engine', 'initialization', 'engine_init_start', 
                     TelemetryLevel.INFO, {
                         'config_path': config_path,
                         'pressure_limit': self.config.pressure_limit,
                         'zone_thresholds': self.config.zone_thresholds
                     })
        ...
        if TELEMETRY_AVAILABLE:
            log_event('pulse_engine', 'initialization', 'engine_init_complete', 
                     TelemetryLevel.INFO, {
                         'zones_initialized': len(self.zones),
                         'batch_size': self.batch_size,
    ...
    def update(self, temperature: float, burn_rate: float) -> PulseEngineEvent:
        ...
        if not self.active:
            if TELEMETRY_AVAILABLE:
                log_event('pulse_engine', 'update', 'update_inactive', 
                         TelemetryLevel.WARN, {
                             'temperature': temperature,
                             'burn_rate': burn_rate
                         })
        ...
        # Log pulse update with comprehensive telemetry
        if TELEMETRY_AVAILABLE:
            log_event('pulse_engine', 'update', 'pulse_update', 
        ...
        with self.config_lock:
            with create_performance_context('pulse_engine', 'update', 'pulse_processing') as perf_ctx:
                perf_ctx.add_metadata('temperature', temperature)
                perf_ctx.add_metadata('burn_rate', burn_rate)
                perf_ctx.add_metadata('buffer_size', len(self.batch_buffer))
                ...
                if (len(self.batch_buffer) >= self.batch_size or 
                    ...
                    perf_ctx.add_metadata('processing_method', 'batch')
                    perf_ctx.add_metadata('batch_size', len(self.batch_buffer))
                else:
                    event = self._process_single_update(temperature, burn_rate)
                    perf_ctx.add_metadata('processing_method', 'single')
                    
                # Log zone transitions if they occurred
                if event and event.zone_event:
                    if TELEMETRY_AVAILABLE:
                        log_event('pulse_engine', 'zone_transition', 'zone_changed', 
                                 TelemetryLevel.INFO, {
                ...
                # Log thermal events if they occurred
                if event and event.thermal_event:
                    level = TelemetryLevel.WARN if event.is_critical else TelemetryLevel.INFO
                    if TELEMETRY_AVAILABLE:
                        log_event('pulse_engine', 'thermal', 'thermal_event', 
                                 level, {
                                     'event_type': event.thermal_event.event_type,
                                     'temperature': temperature,
                                     'burn_rate': burn_rate,
