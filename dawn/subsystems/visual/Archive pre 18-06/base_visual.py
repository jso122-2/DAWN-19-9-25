...
class BaseVisualProcess:
    ...
    def _process_data(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Process data (default implementation)
        ...
        """
        # Default implementation - pass through with basic processing
        if not data:
            return None
        ...
        processed_data = {
            ...
            'original_data': data,
            'processing_metrics': {
                'fps': self.state.metrics.get('fps', 0.0),
                'latency': self.state.metrics.get('latency', 0.0),
                'processor': self.__class__.__name__
            }
        }
        
        # Add any visual-specific processing here
        if 'visual_data' in data:
            processed_data['visual_processed'] = True
            processed_data['visual_size'] = len(str(data['visual_data']))
