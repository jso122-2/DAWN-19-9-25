...
try:
    ...
    print("Warning: pygame not available. Using matplotlib renderer only.")

# Dynamic matplotlib renderer import to avoid circular dependencies
def _get_matplotlib_renderer():
    """Dynamic import to avoid circular dependencies"""
    try:
        from dawn.subsystems.visual.matplotlib_bloom_renderer import MatplotlibBloomRenderer, create_matplotlib_bloom_renderer
        return MatplotlibBloomRenderer, create_matplotlib_bloom_renderer, True
    except ImportError:
        return None, None, False

# MATPLOTLIB_RENDERER_AVAILABLE is determined dynamically by _get_matplotlib_renderer()
...
class BloomVisualization:
    """Handles the visual rendering of the bloom genealogy network with both pygame and matplotlib support"""
    
    def __init__(self, width: int = 1400, height: int = 900, data_source='demo', 
                 renderer_type='matplotlib', export_frames: bool = False):
        """
        Initialize bloom visualization with choice of renderer
        
        Args:
            width: Canvas width
            height: Canvas height 
            data_source: Data source type ('demo', 'stdin', etc.)
        ...
        self.frame_export_counter = 0
        
        # Check available renderers
        print("🔍 DEBUG: About to call _get_matplotlib_renderer()...")
        try:
            result = _get_matplotlib_renderer()
            print(f"🔍 DEBUG: _get_matplotlib_renderer() returned: {result}")
            _, _, matplotlib_available = result
        except Exception as e:
            print(f"🔍 DEBUG: Exception in _get_matplotlib_renderer(): {e}")
            matplotlib_available = False
        
        print(f"🔍 Renderer availability check:")
        print(f"   Matplotlib: {'✅' if matplotlib_available else '❌'}")
        print(f"   Pygame: {'✅' if PYGAME_AVAILABLE else '❌'}")
        
        # Initialize chosen renderer
        if renderer_type == 'matplotlib' and matplotlib_available:
            self._init_matplotlib_renderer()
        elif renderer_type == 'pygame' and PYGAME_AVAILABLE:
            self._init_pygame_renderer()
        elif renderer_type == 'matplotlib' and not matplotlib_available:
            print("⚠️  Matplotlib not available, trying fallback ASCII renderer...")
            self.renderer_type = 'ascii'
            self._init_ascii_renderer()
        else:
            # Fallback to any available renderer
            if matplotlib_available:
                print("🔄 Falling back to matplotlib renderer")
                self.renderer_type = 'matplotlib'
                self._init_matplotlib_renderer()
                ...
                print("🔄 Falling back to ASCII renderer")
                self.renderer_type = 'ascii'
                self._init_ascii_renderer()
        
        print(f"🎨 Using {self.renderer_type} renderer for bloom visualization")
    
    def _init_matplotlib_renderer(self):
        """Initialize matplotlib-based rendering"""
        MatplotlibBloomRenderer, create_matplotlib_bloom_renderer, _ = _get_matplotlib_renderer()
        
        if create_matplotlib_bloom_renderer:
            self.matplotlib_renderer = create_matplotlib_bloom_renderer(
                style="consciousness_flow",
                figure_size=(self.width/100, self.height/100),  # Convert pixels to inches
                enable_glow=True
            )
        else:
        ...
        # Common initialization for both renderers
        self._init_common_properties(data_source)
    
    def _init_pygame_renderer(self):
        """Initialize pygame-based rendering (legacy)"""
        pygame.init()
        ...
        # Common initialization for all renderers
        if not hasattr(self, 'network'):
            self._init_common_properties(data_source)
    
    def _init_ascii_renderer(self):
        """Initialize ASCII art fallback renderer"""
        print("📝 Initializing beautiful ASCII bloom renderer...")
        ...
        # ASCII renderer properties
        self.ascii_canvas_width = 80
        self.ascii_canvas_height = 24
        ...
        # Ensure common properties are initialized
        if not hasattr(self, 'time'):
            self.time = 0.0
        if not hasattr(self, 'frame_count'):
            self.frame_count = 0
        if not hasattr(self, 'network'):
            self.network = BloomGenealogyNetwork(self.width - 300, self.height)
    
    def _init_common_properties(self, data_source):
        """Initialize properties common to all renderers"""
        # Network
        self.network = BloomGenealogyNetwork(self.width - 300, self.height)
    ...
    def detect_bloom_creation(self, json_data: dict) -> List[Dict]:
        ...
        # High entropy + coherence = conceptual bloom (lowered thresholds for demo)
        coherence = scup.get('coherence', 0.5)
        if entropy > 0.4 and coherence > 0.4:
            new_blooms.append({
                'type': 'conceptual',
        ...
        # High heat = sensory/procedural bloom (lowered threshold for demo)
        if heat > 0.5:
    ...
    def process_dawn_state(self, json_data: dict):
        ...
        # In demo mode, create some guaranteed blooms if none detected
        if not new_blooms and len(self.network.blooms) < 20:
            # Force create demo blooms
            bloom_types = ['sensory', 'conceptual', 'emotional', 'procedural', 'meta']
            bloom_type = random.choice(bloom_types)
            new_blooms.append({
                'type': bloom_type,
                'trigger': f'demo_{bloom_type}',
                'strength': random.uniform(0.4, 0.9)
            })
        
        for bloom_data in new_blooms:
            # Random chance based on generation rate (increased for demo)
            gen_rate = BLOOM_TYPES[bloom_data['type']]['generation_rate']
            demo_boost = 0.3  # Increase chance in demo mode
            if random.random() < (gen_rate + demo_boost):
    ...
    def draw(self):
        """Draw the complete visualization using the selected renderer"""
        if self.renderer_type == 'matplotlib':
            self._draw_matplotlib()
        elif self.renderer_type == 'ascii':
            self._draw_ascii()
        else:
            self._draw_pygame()
    
    def _draw_matplotlib(self):
        """Draw using beautiful matplotlib renderer"""
        if self.matplotlib_renderer and self.network:
            # Set the network in the renderer
            self.matplotlib_renderer.network = self.network
            
            # Render with beautiful matplotlib styling
            self.matplotlib_renderer.render_bloom_network(
                network=self.network,
                highlight_active=True,
                show_genealogy=self.show_connections,
            ...
            if self.export_frames:
                import os
                os.makedirs("bloom_frames", exist_ok=True)
                filename = f"bloom_frames/frame_{self.frame_export_counter:06d}.png"
                success = self.matplotlib_renderer.save_frame(filename, high_quality=True)
                if success:
                    print(f"💾 Exported frame {self.frame_export_counter} to {filename}")
                self.frame_export_counter += 1
    
    def _draw_ascii(self):
        """Draw using beautiful ASCII art renderer"""
        print("\n" + "="*80)
        print("🌸 DAWN Beautiful Bloom Network - ASCII Mode 🌸")
        print("="*80)
        
        if not self.network or not self.network.blooms:
            print("📝 No blooms to display")
            return
        
        # Display bloom statistics
        total_blooms = len(self.network.blooms)
        active_blooms = len([b for b in self.network.blooms.values() if b.activation_level > 0.1])
        
        print(f"💫 Total Blooms: {total_blooms} | Active: {active_blooms}")
        print("-" * 80)
        
        # Group blooms by type for beautiful display
        bloom_types = {}
        for bloom in self.network.blooms.values():
            if bloom.bloom_type not in bloom_types:
                bloom_types[bloom.bloom_type] = []
            bloom_types[bloom.bloom_type].append(bloom)
        
        # Display each bloom type
        for bloom_type, blooms in bloom_types.items():
            symbol = BLOOM_TYPES[bloom_type]['symbol']
            color_name = self._get_color_name(BLOOM_TYPES[bloom_type]['color'])
            
            print(f"\n{symbol} {bloom_type.upper()} BLOOMS ({color_name}) - {len(blooms)} total:")
            
            for bloom in sorted(blooms, key=lambda x: x.activation_level, reverse=True):
                # Activation bar visualization
                activation_bars = "█" * int(bloom.activation_level * 10)
                activation_text = f"{activation_bars:<10} ({bloom.activation_level:.2f})"
                
                # Generation indicator
                gen_text = f"Gen:{bloom.generation:2d}" if bloom.generation > 0 else "Gen: 0"
                
                # Connection info
                connections = len(bloom.parents) + len(bloom.children)
                conn_text = f"Links:{connections:2d}"
                
                print(f"  {symbol} ID:{bloom.id:3d} | {activation_text} | {gen_text} | {conn_text}")
        
        # Network statistics
        print("\n" + "-" * 80)
        analytics = self.network.get_analytics()
        print(f"🔗 Connections: {analytics.get('total_connections', 0)}")
        print(f"👪 Family Lines: {analytics.get('family_lines', 0)}")
        print(f"🌳 Max Generation: {analytics.get('deepest_lineage', 0)}")
        print(f"⚡ Active Families: {analytics.get('active_families', 0)}")
        
        print("="*80)
    
    def _get_color_name(self, rgb_tuple):
        """Convert RGB color to readable name"""
        color_names = {
            (79, 195, 247): "Cyan",
            (129, 199, 132): "Green", 
            (255, 138, 101): "Orange",
        ...
        return color_names.get(rgb_tuple, "Color")
    
    def _draw_pygame(self):
        """Draw using legacy pygame renderer"""
    ...
    def run(self, data_source='demo'):
        ...
        if self.renderer_type == 'ascii':
            self._run_ascii_loop()
        else:
            self._run_pygame_loop()
    
    def _run_ascii_loop(self):
        """ASCII renderer loop - simpler without pygame events"""
        print("🎮 Starting ASCII bloom visualization...")
        ...
        try:
            ...
            while True:
                ...
                # Process demo data every second
                if self.data_source == 'demo':
                    demo_timer += dt
                    if demo_timer >= 1.0:  # Generate demo data every second
                        demo_timer = 0
                ...
                # Draw every few frames to avoid spam
                if frame_count % 10 == 0:
                    self.draw()
                
                frame_count += 1
                time.sleep(0.1)  # 10 FPS for ASCII
                
        except KeyboardInterrupt:
            print("\n🛑 ASCII visualization stopped by user")
    
    def _run_pygame_loop(self):
        """Pygame/matplotlib renderer loop"""
        if not PYGAME_AVAILABLE and self.renderer_type == 'pygame':
            print("❌ Pygame not available for event handling")
            return
        ...
        while running:
            ...
            # Handle events only if using pygame
            if PYGAME_AVAILABLE:
            ...
            # Handle different renderers
            if PYGAME_AVAILABLE and self.clock:
                self.clock.tick(60)  # 60 FPS
            else:
                time.sleep(1/60)  # Fallback timing
...
def main():
    ...
    parser.add_argument('--style', choices=['consciousness_flow', 'scientific', 'artistic', 'minimal'], 
                       default='consciousness_flow', help='Matplotlib rendering style (default: consciousness_flow)')
    parser.add_argument('--export-frames', action='store_true', 
                       help='Export frames as images (matplotlib only)')
    
    args = parser.parse_args()
    
    try:
        print("🌸 Starting DAWN Bloom Genealogy Network Visualization")
        print(f"   Renderer: {args.renderer}")
        print(f"   Style: {args.style}")
        print(f"   Data source: {args.source}")
        
        # Create and run visualization with chosen renderer
        viz = BloomVisualization(args.width, args.height, args.source, args.renderer, args.export_frames)
        
        # Configure matplotlib style if using matplotlib
        if args.renderer == 'matplotlib' and hasattr(viz, 'matplotlib_renderer') and viz.matplotlib_renderer:
            viz.matplotlib_renderer.config.style = args.style
            viz.matplotlib_renderer._setup_artistic_styles()
        
        viz.run(data_source=args.source)
        
    except KeyboardInterrupt:
        print("\n🌸 Bloom genealogy visualization stopped by user")
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
        traceback.print_exc()
    finally:
        if 'viz' in locals() and hasattr(viz, 'matplotlib_renderer') and viz.matplotlib_renderer:
            viz.matplotlib_renderer.close()
