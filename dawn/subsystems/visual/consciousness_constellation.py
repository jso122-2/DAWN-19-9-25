...
class ConsciousnessConstellation(DAWNVisualBase):
    """4D SCUP Consciousness Constellation Visualization using unified DAWN visual base"""
    
    def __init__(self, data_source='demo', projection_mode='sphere', visual_config: Optional[DAWNVisualConfig] = None):
        ...
        # SCUP space configuration using consciousness colors
        self.dimensions = {
            'schema': {'color': self.consciousness_colors['stability'], 'label': 'Schema'},
            'coherence': {'color': self.consciousness_colors['awareness'], 'label': 'Coherence'},
            'utility': {'color': self.consciousness_colors['creativity'], 'label': 'Utility'},
            'pressure': {'color': self.consciousness_colors['chaos'], 'label': 'Pressure'}
        }
        ...
        # Data management (device-agnostic tensor operations)
        ...
        # Device-agnostic tensor storage for SCUP data
        self.scup_tensor_buffer = torch.zeros(2000, 4).to(device)  # [time, SCUP]
        ...
        print(f"🌌 Consciousness Constellation initialized - Device: {device}")
        print(f"   Figure size: {self.config.figure_size}")
        print(f"   Projection mode: {projection_mode}")
        print(f"   Tensor buffer: {self.scup_tensor_buffer.shape} on {self.scup_tensor_buffer.device}")
    
    def process_consciousness_tensor(self, scup_data: torch.Tensor) -> torch.Tensor:
        """
        Process consciousness data using device-agnostic tensor operations
        
        Args:
            scup_data: Tensor of shape [4] containing [Schema, Coherence, Utility, Pressure]
            
        Returns:
            Processed tensor with consistency checks and normalization
        """
        # Ensure tensor is on correct device
        scup_data = scup_data.to(device)
        
        # Validate SCUP dimensions
        if scup_data.numel() < 4:
            # Pad with defaults if insufficient data
            padded = torch.zeros(4).to(device)
            padded[:scup_data.numel()] = scup_data.flatten()[:4]
            scup_data = padded
        elif scup_data.numel() > 4:
            scup_data = scup_data.flatten()[:4]
        
        # Clamp to valid range [0, 1]
        scup_data = torch.clamp(scup_data, 0.0, 1.0)
        
        # Check for NaN values
        if torch.isnan(scup_data).any():
            print(f"⚠️ NaN detected in consciousness data, using fallback")
            scup_data = torch.tensor([0.5, 0.5, 0.5, 0.5]).to(device)
        
        # Store in tensor buffer
        self.scup_tensor_buffer[self.current_index] = scup_data
        self.current_index = (self.current_index + 1) % self.scup_tensor_buffer.shape[0]
        if self.current_index == 0:
            self.buffer_full = True
        
        return scup_data
    
    def get_consciousness_trajectory_tensor(self, window_size: int = 100) -> torch.Tensor:
        """Get recent consciousness trajectory as a tensor"""
        if not self.buffer_full and self.current_index == 0:
            # No data yet
            return torch.zeros(window_size, 4).to(device)
        
        if self.buffer_full:
            # Buffer is full, get last window_size entries
            start_idx = (self.current_index - window_size) % self.scup_tensor_buffer.shape[0]
            if start_idx + window_size <= self.scup_tensor_buffer.shape[0]:
                return self.scup_tensor_buffer[start_idx:start_idx + window_size]
            else:
                # Wrap around
                part1 = self.scup_tensor_buffer[start_idx:]
                part2 = self.scup_tensor_buffer[:window_size - len(part1)]
                return torch.cat([part1, part2], dim=0)
        else:
            # Buffer not full yet
            if self.current_index >= window_size:
                return self.scup_tensor_buffer[self.current_index - window_size:self.current_index]
            else:
                # Pad with zeros
                padded = torch.zeros(window_size, 4).to(device)
                padded[window_size - self.current_index:] = self.scup_tensor_buffer[:self.current_index]
                return padded
        
        # Consciousness attractor analysis
    ...
    def render_frame(self, consciousness_data: torch.Tensor) -> plt.Figure:
        """Render a single frame with consciousness data (required by DAWNVisualBase)"""
        # Process the consciousness data
        if consciousness_data is not None and consciousness_data.numel() >= 4:
            self.process_consciousness_tensor(consciousness_data)
        
        # Create or get the figure
        if not hasattr(self, 'fig') or self.fig is None:
            self.fig = self.create_figure((1, 1))
            self.ax = self.fig.add_subplot(111, projection='3d')
            self._setup_3d_space()
        
        # Update visualization with current data
        self.update_visualization(0)  # Call existing update logic
